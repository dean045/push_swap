...
/*   Updated: 2022/01/25 21:39:34 by brhajji-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void sort_suite(t_list **pile_a, t_list **pile_b, t_data data)
{
	...
	while (++i < data.nb[2] - 1)
	{
		while (get_pos(*pile_b, get_best(*pile_a, *pile_b, data)) > ((ft_lstsize(*pile_b)/ 2) )
			&& get_pos(*pile_a, get_next_sup(*pile_a, get_best(*pile_a, *pile_b, data), data)) > ((ft_lstsize(*pile_a)/ 2) ))
		{
			rrr(pile_a, pile_b);
			if ((*pile_b)->content == get_best(*pile_a, *pile_b, data) || (*pile_a)->content == get_next_sup(*pile_a, get_best(*pile_a, *pile_b, data), data))
				break ;
		}
		while (get_pos(*pile_b, get_best(*pile_a, *pile_b, data)) <= ((ft_lstsize(*pile_b)/ 2)) 
		&&  get_pos(*pile_a, get_next_sup(*pile_a, get_best(*pile_a, *pile_b, data), data)) <= ((ft_lstsize(*pile_a)/ 2) )
		&& get_pos(*pile_b, get_best(*pile_a, *pile_b, data)) && get_pos(*pile_a, get_next_sup(*pile_a, get_best(*pile_a, *pile_b, data), data)))
		{
			rr(pile_a, pile_b);
			if ((*pile_b)->content == get_best(*pile_a, *pile_b, data) || (*pile_a)->content == get_next_sup(*pile_a, get_best(*pile_a, *pile_b, data), data))
				break ;
		...
		put_val_top(pile_a, get_next_sup(*pile_a, (*pile_b)->content, data), 'a');
